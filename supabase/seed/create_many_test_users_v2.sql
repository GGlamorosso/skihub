-- ============================================================================
-- Créer BEAUCOUP d'utilisateurs de test pour CrewSnow Bêta (Version 2)
-- ============================================================================
-- 
-- Cette version crée automatiquement les utilisateurs s'ils n'existent pas
-- dans public.users (mais ils doivent exister dans auth.users)
--
-- INSTRUCTIONS :
-- 1. Créez les 22 comptes dans Supabase Dashboard > Authentication > Users
-- 2. Exécutez d'abord : supabase/seed/check_and_create_users.sql
-- 3. Exécutez ensuite ce fichier
--
-- ============================================================================

-- Fonction helper pour créer un utilisateur (version améliorée)
...
BEGIN
    ...
    SELECT EXISTS(SELECT 1 FROM auth.users WHERE id = p_user_id), email
    INTO v_user_exists_in_auth, v_email
    ...
    SELECT EXISTS(SELECT 1 FROM public.users WHERE id = p_user_id) INTO v_user_exists_in_public;
    ...
    IF NOT v_user_exists_in_public THEN
        ...
        ON CONFLICT (id) DO UPDATE SET
            ...
            ride_styles = EXCLUDED.ride_styles,
    ...
    RAISE NOTICE '✅ Utilisateur créé/mis à jour : %', p_username;
END;
$$ LANGUAGE plpgsql;
...
SELECT create_test_user(
    ...
    'expert'::user_level,
    ARRAY['snowboard', 'freestyle', 'park']::ride_style[],
...
SELECT create_test_user(
    ...
    'advanced'::user_level,
    ARRAY['snowboard', 'freeride']::ride_style[],
...
SELECT create_test_user(
    ...
    'intermediate'::user_level,
    ARRAY['snowboard', 'freestyle']::ride_style[],
    ARRAY['en', 'fr']::language_code[],
